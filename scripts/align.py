...
def fold(w):
    ...
    return unicodedata.normalize(
        ...
        "".join(
            ch
            for ch in unicodedata.normalize("NFD", w.lower())
...
def debug_pair(a, b):
    ...
    for c, d in zip(a, b):
        print(
            hex(ord(c)), unicodedata.name(c), unicodedata.name(d) if d else "", sep="\t"
        )
...
def align_tagging(base_path, oga_path, glaux_path, gorman_path, output_filename):
    ...
    with open(f"aligned-tagging/{output_filename}", "w") as g:
        ...
        for token_base, token_oga, token_glaux, token_gorman in zip_longest(
            ...
            if not empty(token_glaux):
                ...
                if glaux_crasis:
                    if BREAK:
                        assert form_glaux.startswith(glaux_crasis), (
                            form_glaux,
                            glaux_crasis,
                        )
...
def align(group_label, group_id, work_id, no_gorman=False):
    align_tagging(
        ...
        None
        if no_gorman
        else f"tagged-texts/{group_id}_{group_label}/{work_id}/tlg{group_id}.tlg{work_id}.gorman.tsv",
